...
|---------|-------|---------------------------------|-----------|
| **1.6J** | **37** | 39 | 2 — `GetLog`/`LogStatusNotification`, one design decision ([D2.2](#62-d2--type-completeness-audit)) |
| **2.0.1** | **64** | 64 | **complete** |
| **2.1** | **91** | 91 | **complete** |
...
| Hardware abstraction | 🚧 | **Seventeen traits.** `ChargePoint`/`Evse`/`Connector` mandatory; `Storage`, `FileTransfer`, `FirmwareInstaller`, `FirmwarePublisher`, `FirmwareVerifier`, `CertificateStore`, `KeyStore`, `SoftwareCrypto`, `OcspClient`, `Iso15118Controller`, `Display`, `BatterySwapStation`, `PaymentTerminal`, `Watchdog` opt-in, each with a `No*` default. [`docs/INTEGRATORS.md`](INTEGRATORS.md) describes them. Still 🚧 only because the surface is **not frozen** ([H5.5](#105-h5--release)) — every functional block now has one. |
...
- [x] **B3.3** Signed firmware verification, on **2.x and 1.6J**.

      `hardware::FirmwareVerifier` (with a **fail-closed** `NoFirmwareVerifier` default) is the
      crypto boundary. Verification is delegated whole — chain check, hash, signature — because
      `FileTransfer::download`'s bytes never cross into this crate and there is no X.509 or
      asymmetric crypto here; the integrator already holds the image. The firmware worker runs the
      check **strictly between `Downloaded` and the install wait**, adding
      `SignatureVerified`/`InvalidSignature` statuses and raising `InvalidFirmwareSignature` /
      `InvalidFirmwareSigningCertificate` on failure. An image that does not verify is never
      installed, and one that *cannot* be verified fails closed.

      **The 1.6J half was expected to be blocked and was not.**
      [D2.2](#62-d2--type-completeness-audit) recorded the security whitepaper types as absent
      upstream; `ocpp-client` 0.5.0 generates `SignedUpdateFirmware` and
      `SignedFirmwareStatusNotification`, so both are wired. See D2.2, now corrected.

      **Breaking change** (not reported by the implementer, caught in review):
      `ChargePointBuilder::firmware_updates` gained a required `verifier: V` parameter and a
      `SignedUpdateFirmwareHandler` bound on `N`.

      **Not done:** pre-download certificate-chain validation (2.x's
      `InvalidCertificate`/`RevokedCertificate`) — the accept/reject response goes out before any
      download starts, and `FirmwareVerifier` is scoped to checking a *fetched* image.
      `DownloadPaused`/`InstallVerificationFailed` remain unmodelled.
- [x] **B3.4** Local-controller firmware publishing (2.x only).

      `PublishFirmware`/`UnpublishFirmware`/`PublishFirmwareStatusNotification` on 2.0.1 and 2.1,
      in a new `crate::publish_firmware` shaped like `crate::firmware`'s queue-and-worker. The
      *download* half is not new work: it reuses `hardware::FileTransfer` verbatim, including the
      `retryInterval` handling `UpdateFirmware` already had. The new hardware surface is one trait,
      `hardware::FirmwarePublisher` (`publish(checksum) -> urls`, `unpublish(checksum) -> bool`),
      deliberately separate from `FirmwareInstaller` — a charge point can update its own firmware
      without being a local controller, and the reverse, so a single trait would force integrators
      to implement a half they do not have. Gated by a new `firmware-publishing` feature and
      `Capabilities::firmware_publishing`, registered in `CAPABILITY_GATES`/`REFUSAL_GATES`.

      Two design decisions worth recording. **Published images are a bounded set** (default 4)
      rather than one-at-a-time: unlike downloading, publishing is not exclusive, and once full a
      genuinely new checksum is refused rather than silently evicting an image another station is
      still fetching. **`UnpublishFirmware` answers synchronously** against the publisher rather
      than being queued, because OCPP defines no async-progress message for it; a hardware failure
      maps to `DownloadOngoing` as the nearest "retry later" the enum offers, since
      `UnpublishFirmwareStatusEnum` has no failure value at all.

      **Not done, and named rather than implied:** checksum verification never emits
      `InvalidChecksum`/`ChecksumVerified` — this crate has no crypto, the same punt
      [B3.3](#b3--firmware-management-r12) records for signature verification — but the checksum
      *is* handed to `FirmwarePublisher::publish`, so an integrator who can hash verifies there.
      `DownloadScheduled`/`DownloadPaused` are modelled for wire completeness and never emitted
      (the request carries no `retrieveDateTime`, and there is no pause/resume).
      `PublishFirmwareStatusNotification` is not in `TriggerMessage`'s set, consistent with
      `FirmwareStatusNotification` also being absent from it today.
...
- [x] **B4.4** OCSP status checking — `GetCertificateStatus` (2.0.1 + 2.1) and
      `GetCertificateChainStatus` (2.1 only).

      OCSP means talking to a third-party responder over HTTP — not the CSMS connection this crate
      owns, and not something `ocpp-client` provides — so the deliverable is the smallest honest
      integration surface: `hardware::OcspClient` with a `No*` default and builder-only
      registration, following `FileTransfer`/`CertificateStore`/`KeyStore`. **A station that
      cannot check reports that it could not**, rather than reporting the certificate good; the
      inverted version of that would be a security defect rather than a missing feature.

      CRL-based revocation is deliberately out: a `GetCertificateChainStatus` entry naming the CRL
      source reports `Failed` honestly instead of fabricating a check.
- [x] **B4.5** ISO 15118 Plug & Charge — gated behind a feature flag *and* a runtime capability.

      `Get15118EVCertificate` on 2.0.1 and 2.1. **This was the last unwired message in either 2.x
      version** — both are now complete at 64/64 and 91/91.

      The EXI request and response are carried **opaquely**: no codec, no 15118 session or
      pagination state machine, nothing deciding when to request a certificate. Not a shortcut —
      ISO 15118 is a separate protocol stack over different physical hardware (PLC/SLAC to the
      vehicle), and speaking it is the one thing this crate cannot do. So
      `hardware::Iso15118Controller` (with an honest-refusal `NoIso15118Controller`) is where
      delivering the CSMS's answer back to the vehicle-facing stack is delegated, following
...
- [x] **B5.3** Monitoring report generation, chunked like `NotifyReport`
      already is.

      `SetMonitoringBase`, `SetMonitoringLevel`, `GetMonitoringReport` and
      `NotifyMonitoringReport` on 2.0.1 and 2.1, on top of [B5.2](#b5--diagnostics-and-monitoring-r14)'s
      existing `VariableMonitorStore` rather than a second copy of the same state.
      `GetMonitoringReport` filters by `monitoringCriteria`/`componentVariable` with the same OR
      semantics `crate::reporting::handle_get_report` uses, groups matches per
      (component, variable), and chunks the answer through the *same* `seqNo`/`tbc`/`requestId`
      scheme and `REPORT_CHUNK_SIZE` constant `crate::reporting::chunk_report` established — one
      chunking scheme in the crate, not two. Registered through a new
      `ChargePointBuilder::monitoring_reports`, kept separate from `::variable_monitoring` the way
      `device_model`/`configuration` already split.

      The two setters **do something**, which is the part worth checking rather than assuming.
      `SetMonitoringBase(FactoryDefault | HardWiredOnly)` actually clears every CSMS-installed
      monitor; `SetMonitoringLevel`'s severity threshold is enforced live in
      `run_variable_monitor_events`/`run_periodic_variable_monitors` before each `NotifyEvent`,
      not merely stored. A handler that returned `Accepted` and changed nothing would be lying to
      the CSMS.

      **Two honest caveats.** `FactoryDefault` and `HardWiredOnly` both reduce to "clear every
      CSMS monitor" because this crate has no concept of hardwired or factory-preconfigured
      monitors to revert *to* — a real difference from the spec's intent, and it stays until such
      monitors exist. And the live severity gate inside the two loop functions has **no test**:
      `VariableMonitorStore::is_reportable` is unit-tested directly, but neither loop has a mock
      notifier fixture (none existed before this task either), so an inverted comparison there
      would not be caught. Named because the behaviour is claimed above.
...
- [x] **B5.6** 2.1 periodic event streams.

      All five: `OpenPeriodicEventStream`, `ClosePeriodicEventStream`, `AdjustPeriodicEventStream`,
      `GetPeriodicEventStream` inbound and `NotifyPeriodicEventStream` outbound, 2.1-only. A new
      `PeriodicEventStreamStore` (bounded by a new `StateLimits::max_periodic_event_streams`,
      default 4) mutated only through three new `ChargePointEvent` variants, mirroring
      `TariffStore`/`DisplayMessageStore`. `run_periodic_event_streams` is the driver — a
      fixed-interval sweep shaped like `run_periodic_variable_monitors`/`run_reservation_expiry`,
      and **skipped while the clock is unsynchronized**, since a station that does not know the
      time should not be stamping periodic samples with a guess.

      Upstream naming trap, confirmed rather than assumed: `ocpp-client` 0.5.0 generates the
      outbound action as `NotifyPeriodicEventStreamAction` (the bare name collides with its own
      payload type) while the method stays `send_notify_periodic_event_stream`. This is exactly
      what defeated the old Appendix A name-match sweep — see its reconciliation note.

      Gated behind the previously-inert `periodic-event-stream` feature and
      `Capabilities::periodic_event_stream`, now wired into `CAPABILITY_GATES` and
      `REFUSAL_GATES`. The refusal shapes split by response schema: `Open`/`Adjust` refuse through
      `GenericStatusEnum`, while `Close`/`Get` must refuse with a CALLERROR because neither
      response type has a status field to say no with.

      **Not done:** OCPP's `values` batching parameter (how many samples to carry per send) is
      parsed, stored and reported faithfully via `GetPeriodicEventStream`, but the driver always
      emits one data element per sweep with `pending: 0` — this crate has no sample-buffering
      infrastructure to batch on top of yet. Documented at the module level, not silent.
...
- [x] **B8.3** Battery swap (2.1): `BatterySwap`, `RequestBatterySwap`.
      ...
      `BatterySwap` outbound and `RequestBatterySwap` inbound, behind the `battery-swap` feature
      and the `battery_swap` runtime capability (both of which existed as inert placeholders and
      now gate real code). State is a bounded `BatterySwapStore` of `PendingBatterySwap` requests
      correlated by OCPP's `requestId` with reported `BatterySwapEvent`s
      (`BatteryIn`/`BatteryOut`/`BatteryOutTimeout`). It tracks only the half that must survive
      between the two CALLs, because OCPP defines no wire-level "swap in progress" status to
      mirror — inventing one would be modelling something the protocol does not have.

      `soC`/`soH` are stored as formatted decimal strings rather than `f64` so `ChargePointEvent`/
      `ChargePointEffect` can keep deriving `Eq`, following `TriggeredMonitor::actual_value`'s
      existing precedent rather than weakening the event type for one field. The hardware surface
      is one small trait, `hardware::BatterySwapStation::prepare_swap`, with a
      `NoBatterySwapStation` that refuses honestly. `handle_request_battery_swap` **rolls back the
      pending-store entry when the station's own preparation fails**, so a `Rejected` CALLRESULT
      never leaves a dangling entry behind it. Builder-only registration
      (`ChargePointBuilder::battery_swap`), matching certificates and display messages, since it
      needs hardware `setup()`'s signature cannot receive.

      `ctrlr_component` is `None` in `CAPABILITY_GATES`, and the reason generalizes well beyond
      this row: **`docs/OCPP-2.1/` is gitignored** (`.gitignore:18`), so it exists only in a
      working copy someone populated by hand. Any fresh clone, CI job or git worktree sees a
      `docs/` with no spec material in it at all. The agent that wrote this block was working in a
      worktree, could not open the appendix, and correctly declined to guess a `*Ctrlr` name —
      taking semantics from the generated `ocpp-types` 0.3.0 source instead, which is
      schema-derived and authoritative for message shape, but says nothing about device-model
      component naming.

      This is worth fixing at the root rather than per-row. Every "verify vs the vendored spec"
      instruction in these documents — including [H3.5](#103-h3--compliance)'s whole sweep and
      [H3.2](#103-h3--compliance)'s part-6 checklist — silently assumes a directory that is not in
      the repository. Either vendor the appendices for real (licensing permitting) or state
      plainly that they must be fetched separately and name where from.
...
- [x] **C4.2 / C4.3** Feature-gate each registration call so an excluded block contributes no
      bounds, and gate registration on runtime capability so one image serves two hardware
      variants.

      Both had been "unblocked, not done" since [C4.1](#54-c4--builder-refactor). The case became
      concrete rather than theoretical: `setup()`'s bound list grew 24 → 45, and
      [B8.2](#b8--reservation-derv2x-battery-swap) declined to add eight more, registering
      builder-only instead — a design decision driven by exactly the limitation these rows exist
      to remove. Every `ChargePointBuilder` registration method is now behind its block's Cargo
      feature, and `CAPABILITY_GATES`'s `has_handler` column tells the truth per row.

      **Scope boundary, stated plainly:** only the *registration methods* are gated, not the
      underlying block modules. `crate::smart_charging`, `crate::variable_monitoring`,
      `crate::certificates` and others are referenced unconditionally from core files as
      protocol-version-independent state infrastructure, so module-level gating is a much larger
      cross-cutting change. C4.2's "no code" half is therefore only partly delivered — the bounds
      go, the modules stay.

      **One claim did not survive review**: that this also fixed a pre-existing compile break in
      `--no-default-features --features ocpp_2_1`. Verification built that configuration on the
      parent commit and found it already clean. Recorded because the row would otherwise carry
      credit for a fix that was not needed.
...
- [x] **D2.1** Diff `ocpp-types` v21's 90 request types against the 2.1
      ...
      such in the audit.)
- [x] **D2.2** 1.6J security whitepaper extensions — **seven of nine wired; two cut for a real
      reason.** 1.6J is now 37 of 39.

      This row's original premise ("absent from `ocpp-client`'s 1.6 action list entirely — decide:
      contribute upstream or declare 1.6J security out of scope") was **stale, and this document
      was still quoting it as current.** All ten are generated by the pinned 0.5.0;
      [B3.3](#b3--firmware-management-r12) had already wired two while *expecting* to be blocked.

      Wired here against the existing protocol-agnostic cores — adapters, not new machinery:
      `SecurityEventNotification`, `InstallCertificate`, `DeleteCertificate`,
      `GetInstalledCertificateIds`, `SignCertificate`, `CertificateSigned`,
      `ExtendedTriggerMessage`.

      **The projection decisions are the substance**, and 1.6J's shapes genuinely differ:
      `CertificateType` names only two roots; `GetInstalledCertificateIds` takes exactly one
      required type and answers flat and type-less; and `SignCertificate`/`CertificateSigned`
      carry **no `certificateType` field at all**. So an inbound `CertificateSigned` is treated as
      the charging-station purpose, and an outbound V2G-purpose `SignCertificate` is refused
      locally (`SignCertificateError::UnsupportedPurpose`) rather than sent unqualified.
      `ExtendedTriggerMessage` reuses the existing `TriggerableMessage` core; its wire enum is a
      *distinct type* from `TriggerMessage`'s (7 variants vs 6), so a trigger valid for one cannot
      reach the other's handler — structurally, not by convention.

      **`GetLog`/`LogStatusNotification` were cut, and that is the interesting part.** 1.6J
      already has `GetDiagnostics`/`DiagnosticsStatusNotification` wired onto *the same*
      `GetLogHandler`/`LogStatusNotifier` traits, and only one `impl GetLogHandler for
      OCPP1_6Client` can exist — a second is a coherence conflict, not a missing adapter. Making
      both work needs a real story about which mechanism a 1.6J station uses when. Cut whole
      rather than half-wired: a message that answers wrongly is worse than an honest
...
- [x] **F1.3** Profile 3 — mutual TLS with a charge point certificate. **This completes
      [M4](#m4--security-and-remote-management)'s "security profiles 1–3" exit criterion.**

      `crate::mutual_tls` builds a `rustls::ClientConfig` presenting the chain installed for
      `CertificateUse::ChargingStation`, signing the handshake through
      `hardware::KeyStore::sign` via rustls's own `SigningKey`/`Signer` delegation pair and
      `with_client_cert_resolver`. **No key-export path was added to `KeyStore`** — that was the
      one outcome that would have defeated [F2.4](#82-f2--tls), and the trait is unchanged.
      Feature-gated on `websocket`+`certificates`+`key-storage`, so no_std builds are unaffected.

      **The real difficulty, and a live constraint on integrators.** rustls 0.23's `Signer::sign`
      is synchronous; `KeyStore::sign` is async by design, because a real secure element's
      operation is a round trip. The bridge is `block_in_place` + `Handle::block_on`, which
      **requires a multi-thread Tokio runtime** — `block_in_place` panics on `current_thread`.
      That is documented at the module level. Closing it properly needs an async `Signer` in
      rustls itself.

      `client_config()` **refuses outright** when no charging-station certificate is installed —
      no silent downgrade to a weaker profile (which would be a security defect) and no bricking;
      the integrator decides whether to retry, refuse or alert. `SecurityProfile::is_implemented`
      now honestly reports all three, with a new `is_usable(has_certificate)` answering the
      station-specific question. Profile 3 sends no Basic auth, with a test asserting it.

      **Three honest gaps.** `StoredCertificates` still cannot hold a charging-station certificate
      (a pre-existing [B4.3](#b4--certificates-and-iso-15118-r1-r13) limitation — its `install`
      refuses that use), so the success path is exercised against an in-test store rather than the
      crate's production one; **profile 3 is therefore not yet end-to-end runnable with the
      built-in store**. There is no live TLS handshake test. And `client_config()` is not wired
      into `connect_and_setup` automatically — the caller supplies it, matching how profile 2's
      TLS already works.
...
- [x] **F2.2** Trust store management fed by [B4](#b4--certificates-and-iso-15118-r1-r13)'s
      installed certificates — **and the bug that made security profile 3 unusable.**

      **The bug first, because it mattered more.** [F1.3](#81-f1--security-profiles)'s own report
      flagged that `StoredCertificates` — the crate's `Storage`-backed default — refuses to hold a
      charging-station certificate, so a station using the built-in store **could not run profile
      3 at all**, however configured, and `mutual_tls::client_config()`'s success path could only
      be exercised against a test double. Fixed, with a test driving `client_config()` against the
      **real** store. The deliberate no-X.509-parsing limitation is preserved: the station's own
      certificate gets a self-computed stand-in hash rather than a parsed one.

      *Worth knowing about that stand-in:* it sets `issuer_name_hash`, `issuer_key_hash` and
      `serial_number` to the same SHA-256 of the PEM. Documented as non-spec, but a CSMS calling
      `GetInstalledCertificateIds` will see three identical hash fields for the station's own
      certificate, which reads as a bug in a wire trace. A proper fix needs X.509 parsing this
      crate does not have.

      **Then the trust store**: `crate::trust_store` builds a `rustls::RootCertStore` from the
      installed `CSMSRootCertificate` set and stages it onto the connection through the same
      `ConnectionTarget` path [F2.1](#82-f2--tls)/[F1.3](#81-f1--security-profiles) already use.
      An empty trust store means trust *nothing*, not everything.

      **Not auto-wired:** nothing rebuilds the trust store when `InstallCertificate`/
      `DeleteCertificate` touches a CSMS root — a caller invokes `build_root_cert_store` +
      `stage_tls_config` after a store change, deliberately mirroring how
      `run_network_profile_switching` is called rather than self-triggering. The "CSMS deleted the
      root we are connected through" case therefore still needs whoever calls it to decide.
...
- [x] **F5.1** Threat model document — a certification auditor will ask.
      [`docs/THREAT-MODEL.md`](THREAT-MODEL.md), linked from the README.

      Assets, trust boundaries, threat actors, and per-threat: what mitigates it today, what does
      not, and which roadmap task covers the gap — cross-referenced by task ID so it stays
      maintainable rather than being a snapshot. Grounded in real code:
      [F5.2](#85-f5--hardening)'s partial payload guard, [F5.3](#85-f5--hardening)'s replay
      detection and what it deliberately does *not* flag, [F2.4](#82-f2--tls)'s no-export
      invariant, the security profiles, and what `crate::persistence` writes in the clear.

      **One overclaim was caught in review and corrected**: it counted `SecurityLogWasCleared`
      among events this crate raises. `persistence::clear_security_log` raises it correctly, but
      nothing calls that function — clearing is a CSMS or maintenance action and neither path
      exists. It is now listed separately as wired-but-unreachable. Worth stating why that
      mattered: a threat model that counts a mitigation which cannot fire is exactly what makes an
      auditor discount the rest of the document.
- [x] **F5.2** Reject oversized/malformed payloads before allocation;
      `MemoryExhaustion` event when limits are hit.

      `crate::payload_limit`: a configurable inbound frame ceiling (`PayloadLimits`, set through
      `ChargePointBuilder` and `connect_and_setup`) enforced by a `SizeLimitedStream` that wraps
      `ocpp_client::TransportStream`, plus a `MemoryExhaustion` security event on every refusal.
      Deliberately transport-agnostic — it needs only `TransportStream`, which is behind no Cargo
      feature — so an embedded integrator supplying their own transport to `setup()` wraps it the
      same way without pulling in this crate's WebSocket wiring.

      **"Before allocation" is honoured only partially, and the module says so at length rather
      than claiming the row outright.** Investigating `ocpp-client` 0.5.0's real surface (not the
      roadmap's description of it) established two things: it passes `None` for
      `tokio_tungstenite`'s `WebSocketConfig`, hard-coding tungstenite's 64 MiB message ceiling
      with nothing in `ConnectOptions` reaching it; and its read loop runs
      `serde_json::from_str::<Value>` — a full JSON DOM — before it knows which action a CALL
      even names. Both allocations happen inside `ocpp-client` before any code here runs.

      So on the **initial** dial, which `ocpp_client::connect*` builds, this crate has no hook and
      does not pretend to. On every **redial** it does: `network_switch::ConnectionTarget::dial`
      goes through the public `websocket_transport`, which hands back the raw sink/stream halves
      before a `Client` exists, and an oversized frame there is dropped without ever reaching
      either deserialization. What that actually buys is the *second* and far more expensive
      allocation — the sized Rust structures, where [D2.3](#62-d2--type-completeness-audit)'s
      56 KB `ChargingProfile` lives — not the first. Lowering the 64 MiB frame ceiling and adding
      a pre-parse hook are upstream asks, recorded here rather than worked around locally per
      `CLAUDE.md`.

      Note the **breaking change**: `connect_and_setup` takes a new `Option<PayloadLimits>`
...
- [x] **H4.3** Sustained-throughput test on a multi-EVSE configuration —
      [`tests/multi_evse_throughput.rs`](../tests/multi_evse_throughput.rs).

      Many EVSEs transacting concurrently through one actor mailbox and one CSMS connection:
      mailbox backpressure (bounded by [G4.5](#94-g4--failure-containment)), per-EVSE isolation so
      one faulted connector does not stall the others, and per-EVSE ordering under concurrency.
      Invariant-based assertions rather than wall-clock timings, with a fast default and an opt-in
      longer mode, matching [H4.1](#104-h4--longevity)/[H4.2](#104-h4--longevity).

      **The paired investigation into H4.1's reconnect memory growth did not settle it**, and the
      way it failed is worth recording. The task concluded the leak was in `ocpp-client` at
      ~176 B/reconnect and recommended filing it upstream with its bisect test as the minimal
      reproduction. Independent verification measured **0 B/reconnect on four runs** at 40/1,200/
      1,200/3,000 rounds; a third measurement got 125.3 B/reconnect. Two of three runs show linear
      growth of the same order, one careful re-run at four scales shows none — so the effect is
      real but **conditional on something not yet identified**, and the upstream recommendation is
      withdrawn. What *was* established stands: not fragmentation (the metric counts
      allocated-minus-freed), not `ConnectionTarget`, and not `ocpp-client`'s obvious
      per-reconnect state, each with a reason. Full data in [`docs/MEMORY.md`](MEMORY.md).

      The bisect test also *asserted* it would hit its reconnect target inside a 300 s deadline
      and hard-failed after ten minutes on a loaded machine, throwing away a well-defined
      bytes-per-reconnect ratio because the count fell short. Now fixed to measure over whatever
      it achieves.
...
- [x] **H5.3 / H5.4** Per-version, per-profile support matrix in the README, plus semver/MSRV
      policy and a changelog.

      The matrix is derived from verified data and ships with `scripts/message-coverage.py` so the
      numbers can be regenerated rather than drifting again — which they had: the README's
      capability-feature table was undercounting which features gate real code. It states
      explicitly that a message being *wired* is not the same as a profile being *certifiable*
      (that needs [H3](#103-h3--compliance)).

      `docs/SEMVER.md` covers what semver means pre-1.0 here, what counts as breaking for a trait
      an integrator implements (a new required method does; one with a default does not), and the
      MSRV commitment. `CHANGELOG.md` reconstructs the history that matters, including the six
      breaking changes made so far — `capabilities()`, `set_current_limit`, `Storage`,
...
### M4 — Security and remote management — ✅ complete (2026-08-09)
...
**All four criteria are met.** Log upload landed with
[B5.1](#b5--diagnostics-and-monitoring-r14); variable monitoring end to end with
[B5.2](#b5--diagnostics-and-monitoring-r14)/[B5.3](#b5--diagnostics-and-monitoring-r14);
**signed firmware over the air** with [B3.3](#b3--firmware-management-r12); and **security
profile 3** with [F1.3](#81-f1--security-profiles), which closed a chain four tasks long —
[F2.4](#82-f2--tls) key storage → [B4.3](#b4--certificates-and-iso-15118-r1-r13) CSR round trip →
F1.3 mutual TLS.

**Four caveats, none of which reopens a criterion, all worth knowing before claiming M4 in a
certification context:**

- **Profile 3 is not yet runnable with the crate's own certificate store.**
  `StoredCertificates` refuses to hold a charging-station certificate (a pre-existing B4.3
  limitation), so `mutual_tls::client_config()`'s success path is exercised against a test store.
  An integrator with their own `CertificateStore` is fine; the built-in one needs fixing.
- **Profile 3 requires a multi-thread Tokio runtime.** rustls's `Signer::sign` is synchronous and
  `KeyStore::sign` is async, bridged with `block_in_place`, which panics on `current_thread`.
- **Signature verification is delegated, not performed here.** `FirmwareVerifier` and
  `OcspClient` both hand the actual cryptography to the integrator, consistent with this crate
  carrying no crypto dependency. The `No*` defaults fail closed — but a station whose integrator
  stubs them out has *no* verification, and nothing in this crate can tell.
- **[F2.2](#82-f2--tls) (trust store) and [F3.2](#83-f3--credentials) (renewal) remain open.**
  Neither is an exit criterion, but both are things an operator will expect from a station
  claiming profile 3.
...
`ocpp-client` **0.5.0** generates per version. `scripts/message-coverage.py`
([H5.3](#105-h5--release)) runs this — use it rather than re-deriving by hand.

**Last re-counted 2026-08-09**, after D2.2 and B4.5.
...
**OCPP 2.0.1 and 2.1 are fully wired.** Every message either version defines is
handled. The two remaining are both 1.6J, both deliberate, and both described in
[D2.2](#62-d2--type-completeness-audit).

**Four traps, every one of which silently *understates* coverage.** This sweep
has now hit all four; handle them or the number will be wrong again:

1. **Acronyms.** `GetDERControl` → `on_get_der_control`, not
   `on_get_d_e_r_control`; `AFRRSignal` → `send_afrr_signal`. A per-capital
   split once reported 12 wired messages as missing.
2. **Digits.** `Get15118EVCertificate` → `get_15118_ev_certificate`. A splitter
   handling acronyms but not letter→digit boundaries reports the *last* message
   of 2.x coverage as unwired — which is exactly what happened on the run that
   completed it. Split on `([a-z0-9])([A-Z])`, `([A-Z]+)([A-Z][a-z])` **and**
   `([A-Za-z])([0-9])`.
3. **An `Action` suffix.** `NotifyPeriodicEventStream` is generated as
   `NotifyPeriodicEventStreamAction`; the method keeps the unsuffixed name.
4. **Module names are prefixed.** Adapters are named e.g.
   `clear_cache_ocpp_2_1`, and some live in files under an `ocpp_2_1/` path, so
   detection must be substring-based.

**The check that catches all four**, and it has earned its place: a correct
sweep leaves **no wired method unmatched** by some generated action. Trap 2 was
caught by precisely that assertion firing, not by anyone noticing the count
looked low.

### A.1 OCPP 1.6J — 37 of 39 wired

The core profile has long been complete; the denominator moved when
`ocpp-client` 0.4.0 added the security whitepaper set (28 → 39 actions).
[B3.3](#b3--firmware-management-r12) wired two of that set and
[D2.2](#62-d2--type-completeness-audit) seven more.

**Missing (2):** `GetLog`, `LogStatusNotification`.

Both are **deliberate**, and the reason is structural rather than effort:
1.6J already wires `GetDiagnostics`/`DiagnosticsStatusNotification` onto *the
same* `GetLogHandler`/`LogStatusNotifier` traits, and only one
`impl GetLogHandler for OCPP1_6Client` can exist — a second is a coherence
conflict. Supporting both needs a story about which mechanism a 1.6J station
uses when. See [D2.2](#62-d2--type-completeness-audit).

### A.2 OCPP 2.0.1 — 64 of 64 · complete

### A.3 OCPP 2.1 — 91 of 91 · complete

**Inventory reconciliation.** 37 + 2 = 39, 64 + 0 = 64, 91 + 0 = 91, with no
wired method unmatched.

**What this means for the roadmap.** Message coverage is finished as a
workstream. The two open 1.6J messages are one design decision, not a backlog.
Everything remaining in [Workstream B](#4-workstream-b--message-coverage) is
therefore closed, and production readiness from here is
[Workstream H](#10-workstream-h--test-compliance-release) — compliance runs,
release process, the 1.0 freeze — plus the hardware surfaces DER actuation and
ISO 15118 still need behind their (already wired) messages. **Note the
distinction the README's matrix also draws: a message being wired is not the
same as a profile being certifiable.**
