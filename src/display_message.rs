...
pub async fn handle_clear_display_message(
    ...
    let _ = actor.send(ChargePointEvent::DisplayMessageCleared(id)).await;
...
mod tests {
    ...
    async fn a_supported_format_is_accepted() {
        ...
        let outcome =
            handle_set_display_message(&actor, message(1, MessagePriority::NormalCycle, None), &[
                MessageFormat::Ascii,
            ])
            .await;

        assert_eq!(outcome, SetDisplayMessageOutcome::Accepted);
        assert!(actor.state().display_messages.get(DisplayMessageId(1)).is_some());
    }
    ...
    async fn an_unsupported_format_is_refused_without_storing_it() {
        ...
        assert!(actor.state().display_messages.get(DisplayMessageId(1)).is_none());
    }
    ...
    async fn setting_beyond_the_configured_maximum_is_rejected() {
        ...
        let outcome =
            handle_set_display_message(&actor, message(2, MessagePriority::NormalCycle, None), &[
                MessageFormat::Ascii,
            ])
            .await;
    ...
    async fn no_filter_returns_every_stored_message() {
        ...
        .await;
        handle_set_display_message(&actor, message(2, MessagePriority::AlwaysFront, None), &[
            MessageFormat::Ascii,
        ])
        .await;
    ...
    async fn a_priority_filter_only_matches_that_priority() {
        ...
        match outcome {
            GetDisplayMessagesOutcome::Accepted(messages) => {
                assert_eq!(messages, alloc::vec![message(2, MessagePriority::AlwaysFront, None)]);
            }
            other => panic!("expected Accepted, got {other:?}"),
        }
    }
    ...
    async fn a_message_scoped_to_charging_only_shows_while_charging() {
        ...
        handle_set_display_message(
            &actor,
            message(1, MessagePriority::NormalCycle, Some(MessageState::Charging)),
            &[MessageFormat::Ascii],
        )
        .await;
        ...
        assert_eq!(actor.state().evses[0].connectors[0], ConnectorState::Charging);
        assert_eq!(
...
mod ocpp_2_1 {
    ...
    use ocpp_client::ocpp_types::v21::common::{
        ...
        MessageContent as WireMessageContent, MessageFormatEnum, MessageInfo,
        MessagePriorityEnum, MessageStateEnum,
    };
    ...
    mod tests {
        ...
        fn a_malformed_transaction_id_becomes_a_never_matching_sentinel() {
            assert_eq!(parse_transaction_id("not-a-number"), TransactionId(u64::MAX));
...
mod ocpp_2_0_1 {
    ...
    mod tests {
        ...
        fn a_qr_code_format_projects_to_uri_rather_than_a_wrong_format() {
            assert_eq!(build_message_format(MessageFormat::QrCode), MessageFormatEnum::URI);
        }
