...
mod tests {
    ...
    async fn set_default_tariff_for_an_unknown_evse_is_rejected() {
        ...
        let outcome =
            handle_set_default_tariff(&actor, TariffScope::Evse(99), tariff("t1")).await;
    ...
    async fn get_tariffs_with_nothing_installed_reports_no_tariff() {
        ...
        assert_eq!(handle_get_tariffs(&actor, None), GetTariffsOutcome::NoTariff);
    }
...
pub mod ocpp_2_1 {
    ...
    async fn handle_set_default(
        ...
        let outcome =
            handle_set_default_tariff(actor, scope, wire_tariff(&request.tariff)).await;
    ...
    async fn handle_change_transaction(
        ...
        let outcome = handle_change_transaction_tariff(
            actor,
            transaction_id,
            wire_tariff(&request.tariff),
        )
        .await;
        let status = match outcome {
    ...
    async fn handle_clear(
        ...
        let ids: Option<Vec<TariffId>> = request.tariff_ids.as_ref().map(|ids| {
            ids.iter()
                .map(|id| TariffId(id.as_str().into()))
                .collect()
        });
    ...
    mod tests {
        ...
        fn scope_round_trips_through_the_wire() {
            ...
            assert_eq!(parse_scope(scope_to_wire(TariffScope::Evse(3))), Some(TariffScope::Evse(3)));
        }
